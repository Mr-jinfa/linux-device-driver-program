/*使用读写信号量来做互斥
*读写信号量使用情景:
*一个rwsem只允许一个写者、无数个读者拥有该信号量,因此写者具有更高优先级.
*所以,当有很多个写者同时竞争资源时,读者将会"饿死".
*为此,最好在很少需要写访问且写访问只会短期拥有信号量的时候使用rwsem
*/
...
ssize_t scull_write(struct file *filp, const char __user *usr, size_t len, loff_t *off)
{
	...
	/*一般来说,使用下面这个api将允许挨在后面的读者访问资源 */	
    downgrade_write(&dev->rw_sem);
...
ssize_t scull_read(struct file *filp, char __user *usr, size_t len, loff_t *off)
{
	...
	/*注意下面这个api返回值和大多数内核函数不同*/
    down_read_trylock(&dev->rw_sem);
...
static int __init scull_init(void)
{
    ...
    for(i=0; i< scull_nr_devs; i++)
    {
		...
		init_rwsem(&scull_devices[i].rw_sem);
